...
class Pipeline:
	...
	def work_explore(self, agent1, agent2):
		...
		self.merged_map,self.affine_matrix=self.map_merger.merge_maps(occupancygrid1, occupancygrid2)
		#print(self.merged_map,self.affine_matrix)
		flag=True
		for i in range(1):
			for j in range(1):
				if i==j:
					#print(self.affine_matrix[i][j])
					if not math.isclose(self.affine_matrix[i][j],1,abs_tol=1e-2):
						
						flag=False

				else:
					if not math.isclose(self.affine_matrix[i][j],0,abs_tol=1e-2):
						flag=False


		if flag==True:		 
			self.mergecount+=1
		if self.mergecount>=10:		
			self.mode='Rendezvous'
			self.path_agent1=None
			cv2.imwrite('image3.jpg',self.merged_map)
			print("Maps merged")
		return

	def work_rendezvous(self,agent1,agent2):
		pygame.init()
		
		if self.merged_map_surface==None:
			
			self.ind1=0
			self.merged_map_surface=pygame.Surface((self.world_width*2,self.world_height*2))
			self.merged_map_surface.fill((0,0,0))
			for i in range(2*self.world_height):
				for j in range(2*self.world_width):
					
					
					if (self.merged_map[i][j] != np.array([127, 127, 127])).any() and (self.merged_map[i][j] != np.array([64, 64, 64])).any():
						self.merged_map_surface.set_at((i, j), (255, 255, 255))
			pygame.image.save(self.merged_map_surface,'temp.jpg')
			return
		else:
			tx,ty=(self.affine_matrix[0][2],self.affine_matrix[1][2])
			'''print(int(tx),int(ty))
			print(self.ind1)
			print(agent1.get_pos(),(agent2.get_pos()[0]-int(tx),agent2.get_pos()[1]-int(ty)))'''
			if self.path_agent1==None:
				self.path_agent1=self.planner1.get_path(self.merged_map_surface,self.world_width*2,self.world_height*2,agent1.get_pos(),(agent2.get_pos()[0]-int(tx),agent2.get_pos()[1]-int(ty)))
			print(self.path_agent1)
			
			navigate_to_node(agent1, self.path_agent1[self.ind1])
			if math.dist(agent1.get_pos(),self.path_agent1[self.ind1])<25:
				self.ind1+=1
			if self.ind1>=len(self.path_agent1):
				self.mode="completed"
				print("Ägents should have rendezvous'd")
		return